...
export async function deobfuscate(filename) {
     ...
     var hsw_code = filename 
    
    hsw_code = hsw_code.replace("1.toString(-1)", "")
   ...
   // var start = performance.now()
    ...
    //var hsw_body = ast.program.body[0].declarations[0].init.callee.body.body
    //hsw_body.forEach(path => {
    //    if (path.type == "FunctionDeclaration"){
    //        funcs[path.id.name] = path
    //    } 
    //})
    const state = {
        decoder_map: new Map(),
